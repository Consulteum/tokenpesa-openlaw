 ...
 */
contract WrappedKSHEscrows {
    using SafeMath for uint256;
    
    string public descr;
    uint256 public price;
    WrappedKSH tokenContract;
 ...
 */    
   constructor(
        ...
        public {
                descr = _descr;
                price = _price;
                tokenContract = WrappedKSH(_tokenContract);
                tokenize = _tokenContract;
                buyer = _buyer;
                seller = _seller;
                 ...
                 */
                  modifier onlyArbiter() {
                        require(tokenContract.reputation(msg.sender) >= tokenContract.minReputation());
                         _;
                        }
                /**
         ...
         */
           function confirmReceipt() public onlyBuyer inState(State.Created) {
                ...
                ERC20 token = ERC20(tokenize);
                uint256 bal = token.balanceOf(address(this));
                tokenContract.transfer(seller, bal);
	...
	if(msg.sender == buyer || msg.sender == seller) revert();
              ...
              //  ERC20 token = ERC20(tokenContract);
                tokenContract.transfer(buyer, buyerAward);
                tokenContract.transfer(seller, sellerAward);
                tokenContract.transfer(msg.sender, arbiterFee);

                emit Resolved(address(this), buyer, seller);
                }
}

contract WrappedKSHEscrowsFactory {

  //contract address of the Wrapped KSH token
  ...
  constructor(address wksh) public {
  ...
  function newWrappedKSHEscrow(string memory _descr, uint256 _price, address _buyer, address _seller, uint _arbiterFee) public returns(address)
   {

    WrappedKSHEscrows c = new WrappedKSHEscrows(_descr, _price, _tokenContract, _buyer, _seller, _arbiterFee);

            validContracts[address(c)] = true;
